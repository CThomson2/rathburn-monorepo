...
CREATE UNIQUE INDEX password_reset_tokens_pkey ON auth.password_reset_tokens USING btree (id);

CREATE UNIQUE INDEX password_reset_tokens_user_id_token_key ON auth.password_reset_tokens USING btree (user_id, token);

CREATE UNIQUE INDEX users_email_key ON auth.users USING btree (email);

alter table "auth"."password_reset_tokens" add constraint "password_reset_tokens_pkey" PRIMARY KEY using index "password_reset_tokens_pkey";

alter table "auth"."password_reset_tokens" add constraint "password_reset_tokens_user_id_fkey" FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE not valid;

alter table "auth"."password_reset_tokens" validate constraint "password_reset_tokens_user_id_fkey";

alter table "auth"."password_reset_tokens" add constraint "password_reset_tokens_user_id_token_key" UNIQUE using index "password_reset_tokens_user_id_token_key";
