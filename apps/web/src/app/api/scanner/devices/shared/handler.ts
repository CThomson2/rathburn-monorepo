...
import {
  ...
  updateDeviceStatus,
  logDeviceActivity,
} from "@/app/api/scanner/scan/processor";
  ...
  return async function handler(req: NextRequest): Promise<NextResponse> {
    try {
      ...
      const activityDetails: Record<string, any> = {
        ...
        action_type: validatedData.actionType || "unknown",
      ...
      const actionType =
        validatedData.actionType || detectActionType(validatedData.barcode);
