...
const mockDrums = Array.from({ length: 20 }).map((_, i) => ({
  id: i + 1,
  material: ["Benzene", "Toluene", "Xylene", "Ethanol", "Methanol"][
    Math.floor(Math.random() * 5)
  ] as string,
  supplier: ["Chemco", "ReactiveSupplies", "OrganicChem", "PetroChem"][
    Math.floor(Math.random() * 4)
  ] as string,
  batch_code: `B${Math.floor(Math.random() * 10000)
    .toString()
    .padStart(4, "0")}`,
  fill_level: Math.floor(Math.random() * 200),
  status: ["in_stock", "in_use", "empty", "scheduled", "in_transit"][
    Math.floor(Math.random() * 5)
  ] as string,
  created_at: new Date(
...
export default function DrumManagementPage() {
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedDrum, setSelectedDrum] = useState<any>(null);
  ...
  const handleRowClick = (drum: any) => {
    setSelectedDrum(drum);
  };

  const handleDownloadLabel = (drumId: number) => {
    // Replace with your toast implementation
    console.log(`Label for Drum ID: ${drumId} has been downloaded`);
  };
