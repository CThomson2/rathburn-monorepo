// src/features/production/services/production-service.ts
import { Order, mapJobStatusToOrderStatus, getProgressFromStatus, getPriorityFromJob } from "../types";
import { executeDbOperation } from "@/lib/database";

/**
 * Fetches all production jobs from the database
 */
export const fetchProductionJobs = async (): Promise<Order[]> => {
  try {
    const { data: jobs, error } = await executeDbOperation(async (supabase) => {
      const response = await supabase
        .from('production.jobs')
        .select(`
          job_id,
          item_id,
          input_batch_id,
          status,
          priority,
          planned_start,
          planned_end,
          created_at,
          updated_at,
          inventory_items:item_id (
            name,
            supplier:supplier_id (
              name
            )
          ),
          operations:production.operations (
            op_id,
            op_type,
            status,
            scheduled_start,
            started_at,
            ended_at,
            operation_drums:production.operation_drums (
              drum_id,
              drums:drum_id (
                serial_number,
                current_volume,
                current_location,
                locations:current_location (
                  name
                )
              )
            )
          )
        `)
        .order('created_at', { ascending: false });
      
      return response;
    }, "production.jobs");

    if (error) {
      console.error('Error fetching production jobs:', error);
      return [];
    }

    // Transform the database data into the Order type expected by the UI
    return jobs.map((job: any) => {
      // Count the drums across all operations
      const allDrums = job.operations.flatMap(op => op.operation_drums || []);
      const uniqueDrumIds = new Set(allDrums.map(d => d.drum_id));
      
      return {
        id: `ORD-${job.job_id.substring(0, 8)}`,
        itemName: job.inventory_items?.name || 'Unknown Item',
        supplier: job.inventory_items?.supplier?.name || 'Unknown Supplier',
        quantity: uniqueDrumIds.size,
        scheduledDate: job.planned_start || job.created_at,
        status: mapJobStatusToOrderStatus(job.status),
        progress: getProgressFromStatus(job.status),
        priority: getPriorityFromJob(job),
        drums: allDrums.map(drumOp => ({
          id: drumOp.drum_id,
          serial: drumOp.drums?.serial_number || 'Unknown',
          volume: drumOp.drums?.current_volume || 0,
          location: drumOp.drums?.locations?.name || 'Unknown Location'
        })),
        tasks: job.operations.map(op => ({
          name: `${op.op_type.charAt(0).toUpperCase() + op.op_type.slice(1)} operation`,
          completed: op.status === 'completed',
          assignedTo: '' // No assigned user in the current data model
        })),
        // Add basic timeline based on operation starts/ends
        timeline: job.operations
          .filter(op => op.started_at || op.ended_at)
          .flatMap(op => {
            const events = [];
            if (op.started_at) {
              events.push({
                event: `${op.op_type} started`,
                timestamp: op.started_at
              });
            }
            if (op.ended_at) {
              events.push({
                event: `${op.op_type} completed`,
                timestamp: op.ended_at
              });
            }
            return events;
          })
          .sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime())
      };
    });
  } catch (error) {
    console.error('Unexpected error fetching jobs:', error);
    return [];
  }
};

/**
 * Creates a new production job
 */
export const createProductionJob = async (
  itemId: string,
  batchId: string,
  plannedStart: Date,
  priority: number = 5
): Promise<string | null> => {
  try {
    const { data, error } = await withSupabaseClient(async (supabase) => {
      const response = await supabase
        .from('production.jobs')
        .insert({
          item_id: itemId,
          input_batch_id: batchId,
          planned_start: plannedStart.toISOString(),
          priority: priority,
          status: 'scheduled'
        })
        .select('job_id')
        .single();
      
      return response;
    });

    if (error) {
      console.error('Error creating production job:', error);
      return null;
    }

    // Create initial transport operation for this job
    if (data?.job_id) {
      await createInitialOperations(data.job_id);
      return data.job_id;
    }

    return null;
  } catch (error) {
    console.error('Unexpected error creating job:', error);
    return null;
  }
};

/**
 * Creates the initial set of operations for a new job
 */
const createInitialOperations = async (jobId: string): Promise<boolean> => {
  try {
    // Create transport operation
    const { error } = await withSupabaseClient(async (supabase) => {
      const response = await supabase
        .from('production.operations')
        .insert({
          job_id: jobId,
          op_type: 'transport',
          status: 'pending'
        });
      
      return response;
    });

    if (error) {
      console.error('Error creating initial operations:', error);
      return false;
    }

    return true;
  } catch (error) {
    console.error('Unexpected error creating operations:', error);
    return false;
  }
};

/**
 * Updates an existing production job
 */
export const updateProductionJob = async (
  jobId: string,
  updates: {
    status?: string;
    priority?: number;
    planned_start?: Date;
  }
): Promise<boolean> => {
  try {
    const { error } = await withSupabaseClient(async (supabase) => {
      const updateData: any = {};
      
      if (updates.status) updateData.status = updates.status;
      if (updates.priority !== undefined) updateData.priority = updates.priority;
      if (updates.planned_start) updateData.planned_start = updates.planned_start.toISOString();
      
      const response = await supabase
        .from('production.jobs')
        .update(updateData)
        .eq('job_id', jobId);
      
      return response;
    });

    if (error) {
      console.error('Error updating production job:', error);
      return false;
    }

    return true;
  } catch (error) {
    console.error('Unexpected error updating job:', error);
    return false;
  }
};

/**
 * Fetches available batches that can be used for new jobs
 */
export const fetchAvailableBatches = async () => {
  try {
    const { data, error } = await withSupabaseClient(async (supabase) => {
      const response = await supabase
        .from('ui.v_batches_with_drums')
        .select('*')
        .gt('drums_in_stock', 0);
      
      return response;
    });

    if (error) {
      console.error('Error fetching available batches:', error);
      return [];
    }

    return data;
  } catch (error) {
    console.error('Unexpected error fetching batches:', error);
    return [];
  }
};

/**
 * Fetches all items for job creation
 */
export const fetchItems = async () => {
  try {
    const { data, error } = await withSupabaseClient(async (supabase) => {
      const response = await supabase
        .from('inventory.items')
        .select(`
          item_id,
          name,
          supplier:supplier_id (
            supplier_id,
            name
          ),
          material:material_id (
            material_id,
            name
          )
        `);
      
      return response;
    });

    if (error) {
      console.error('Error fetching items:', error);
      return [];
    }

    return data;
  } catch (error) {
    console.error('Unexpected error fetching items:', error);
    return [];
  }
};