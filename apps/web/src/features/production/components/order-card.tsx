...
import { CheckCircle, AlertTriangle, Loader, ChevronDown, ChevronUp, Edit } from "lucide-react";
...
export const OrderCard = ({ order }: OrderCardProps) => {
  ...
  const getStatusIcon = (status: OrderStatus) => {
    switch(status) {
      ...
      default:
        return <Loader className="text-status-preparing animate-spin" size={18} />;
    }
  };
  ...
  return (
    <div 
      ref={cardRef}
      className={cn(
        "bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 transition-all duration-300",
        expanded ? "mb-8" : "mb-4"
      )}
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      ...
      >
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex items-center">
            <div className="text-right mr-4">
              <p className="text-sm font-medium text-gray-900 dark:text-white">{order.quantity} drums</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{formatDate(order.scheduledDate)}</p>
            </div>
            <button 
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <button 
            className="px-4 py-1.5 text-sm bg-brand-blue text-white rounded-lg hover:bg-blue-600 transition-colors"
            onClick={(e) => {
              e.stopPropagation();
