    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
            Order {order.id}
            <span className={`ml-3 px-3 py-1 text-xs font-medium rounded-full text-white
      ...
      <Tabs defaultValue="overview" className="w-full">
            ...
            <div className="space-y-4">
              <div>
                ...
                {editMode ? (
                  ...
                  <p className="mt-1 text-gray-900 dark:text-white">{order.itemName}</p>
                )}
              </div>
              ...
              <div>
                ...
                {editMode ? (
                  ...
                  <p className="mt-1 text-gray-900 dark:text-white">{order.supplier}</p>
                )}
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Quantity (Drums)</label>
                {editMode ? (
                  <input 
                    type="number"
                    value={editedOrder.quantity}
                    onChange={(e) => setEditedOrder({...editedOrder, quantity: parseInt(e.target.value)})}
                    className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 
                      shadow-sm p-2 bg-white dark:bg-gray-800"
                  />
                ) : (
                  <p className="mt-1 text-gray-900 dark:text-white">{order.quantity}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                {editMode ? (
                  <input 
                    type="datetime-local"
                    value={new Date(editedOrder.scheduledDate).toISOString().slice(0,16)}
                    onChange={(e) => setEditedOrder({...editedOrder, scheduledDate: new Date(e.target.value).toISOString()})}
                    className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 
                      shadow-sm p-2 bg-white dark:bg-gray-800"
                  />
                ) : (
                  <p className="mt-1 text-gray-900 dark:text-white">{formatDate(order.scheduledDate)}</p>
                )}
              </div>
              ...
              <div>
                ...
                {editMode ? (
                  ...
                  <p className="mt-1 flex items-center">
                    {order.status === 'complete' && <CheckCircle size={16} className="text-status-complete mr-1" />}
                    {order.status === 'error' && <AlertTriangle size={16} className="text-status-error mr-1" />}
                    {(order.status !== 'complete' && order.status !== 'error') && 
                      <Loader size={16} className="text-status-preparing mr-1 animate-spin" />}
        ...
        <TabsContent value="tasks" className="space-y-4">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Production Tasks</h3>
              ...
              <div key={index} className="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
                <div className="flex items-center">
                  <input 
                    ...
                    readOnly
                  />
                  <span className={`ml-3 ${task.completed ? 'line-through text-gray-500' : 'text-gray-900 dark:text-white'}`}>
                    {task.name}
                  </span>
                </div>
        ...
        <TabsContent value="drums" className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Assigned Drums</h3>
            ...
            {order.timeline?.map((event, index) => (
              <div key={index} className="flex">
                ...
                <div className="flex flex-col flex-grow pb-6">
                  <div className="flex items-center">
                    <h4 className="text-sm font-medium text-gray-900 dark:text-white">{event.event}</h4>
                    {event.user && (
