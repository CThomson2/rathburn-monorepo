...
// Determine directory paths based on current working directory
const currentDir = process.cwd();
let appDir;
let monorepoRoot;

// Determine if we're running from the monorepo root or the app directory
if (currentDir.endsWith("rathburn-app")) {
  // Running from app directory
  appDir = currentDir;
  monorepoRoot = path.resolve(currentDir, "../..");
} else if (currentDir.endsWith("rathburn-ops-monorepo")) {
  // Running from monorepo root
  monorepoRoot = currentDir;
  appDir = path.join(monorepoRoot, "apps", "rathburn-app");
} else {
  // Running from another directory, try to find the monorepo structure
  if (fs.existsSync(path.join(currentDir, "apps", "rathburn-app"))) {
    monorepoRoot = currentDir;
    appDir = path.join(monorepoRoot, "apps", "rathburn-app");
  } else {
    // Fall back to assuming this script is in the correct app directory
    appDir = __dirname;
    monorepoRoot = path.resolve(appDir, "../..");
  }
}
...
process.env.MONOREPO_ROOT = process.env.MONOREPO_ROOT || monorepoRoot;
process.env.PORT = process.env.PORT || "3000";
process.env.HOST = process.env.HOST || "0.0.0.0";

// Path to standalone server.js - use appDir instead of __dirname to work from any directory
const STANDALONE_PATH = path.join(appDir, ".next", "standalone", "server.js");
...
for (const dep of requiredDependencies) {
  ...
  if (!fs.existsSync(depPath)) {
    console.warn(
      "\x1b[33m%s\x1b[0m",
      `Warning: ${dep} not found in standalone/node_modules`
    );
...
// Start the server
try {
  // Change to app directory to ensure correct path resolution
  if (process.cwd() !== appDir) {
    console.log(`Changing working directory to: ${appDir}`);
