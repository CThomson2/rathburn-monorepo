...
criticalDeps.forEach(dep => {
  ...
  try {
    ...
    console.warn(`⚠️ Error copying ${dep}: ${error.message}`);
  }
});
...
function findDependencyInNodeModules(dep) {
  ...
  try {
    ...
    // Try to find in .pnpm directory structure
    const pnpmDir = path.join(appDir, 'node_modules', '.pnpm');
    if (fs.existsSync(pnpmDir)) {
      // For simplicity, we'll brute force search with a bash command
      const nameToSearch = dep.replace('@', '').replace('/', '+');
      const cmd = `find "${pnpmDir}" -type d -name "node_modules" -exec find {} -maxdepth 1 -type d -name "${dep.replace('@', '')}" \\; | head -n 1`;
    ...
    console.warn(`Error searching for ${dep} in pnpm structure:`, error.message);
  }
